...
* & is stack share_ptr but with safer mut/const + non-null check and can be rawed to C++ ptr, Box<T\> is heap ref == new unique_ptr, Ref<T\> is heap share_ptr
* copy is never deep unless you explicitly do clone
