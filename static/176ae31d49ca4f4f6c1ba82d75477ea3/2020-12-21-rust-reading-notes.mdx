...
[Perform like C++](https://benchmarksgame-team.pages.debian.net/benchmarksgame/which-programs-are-fastest.html) yet "secure" like a script lang. Talks like a cool scripting lang yet powerful enough to go deep down to system level. 

As a more secure and yet efficient lang, Rust recently catches my eyes and [some people](https://hub.packtpub.com/rust-as-a-game-programming-language-is-it-any-good/) even believe it can replace (at least somewhat) the long-live C++ for game dev. Although lang itself is not the one to blame for messy C++ game code, I still wish there is a cleaner/stricter language to branch from C++ (instead of splicing tons of historical/back-compatible features) so that developer doesn't need to create their own code standard, wheels, memory model etc. Junior developers under C++ tends to depise such rules which potentially leading to hard-to-find bugs like dangling pointers, memory tampering, multithread resource sharing with unclear ownership etc.. Rust uses stricter compiling check (historically strict ownership) to make sure a more reliable run time code, which means like a good human language teacher who requires you to elaborate your every sentence. A lot of system level cli app I used are written by Rust. After my reading, I believe knowing rust can help foster lots of good C++ habits that C++ junior won't be told when first diving to C++ project. Learning rust could be the fastest way to know how to be a better C++ coder. Besides, with like other mainstream langs, Rust has friendly pkg manager 👍(cargo and crates.io) than C++. It's definitely another level to old C/C++ eco. Enthusiastic people even consider using it to build safer [OS](https://doc.redox-os.org/book/ch01-02-what-is-redox.html)!

In web world, Rust is also modern and mutually first class to webassembly, if you believe wasm is the future.
...
* homework: try to implement link list using rust (ref https://rust-unofficial.github.io/too-many-lists/second-option.html)
