...
* common methods: len, is_empty, push/pop, append/split_off, dedup, clear, reserve/with_capacity, insert/remove, resize, splice, as_((mut_)slice/ptr)
* Deref methods for binary search etc. like c++ container
...
* common methods for both String/str: len, push(_str)/pop, from(_utf8_lossy), insert(_str)/remove, clear, is_empty, split_off, as(into)_bytes, replace_range, find
