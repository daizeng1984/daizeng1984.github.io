...
```
## Android
Ok, it looks like there's some work to do for this couple when coming to android. Android requires nix-shell's `buildFHSUserEnv` which means it creates chroot sandbox to get path e.g. `/usr/bin/perl`. However, it conflicts with direnv. Therefore mixing them is not a good idea. In this case, I'd assume it's like conda case, we need to use relative path solution.
Ok, direnv is 
## NDK (TODO)
