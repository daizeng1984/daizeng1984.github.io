...
* common methods for both String/str: len, push(_str)/pop, from(_utf8_lossy), insert(_str)/remove, clear, is_empty, split(_off), as(into)_bytes, replace_range, find
...
```
* panic!, Result (unwrap/expect to panic) or ? (return/propagate Result)
