...
+ use std::string_view replacing c style string or even c++ std::string, and get bonus like start_with(), substr() and better performance etc. view is **read only**
...
// String and values
to_chars(any primitive type)
from_chars(first, last, out, base)->from_chars_result = { *ptr, errc::out_of_range }
to_string & stoi // throws

// Tokenize
getline(sstr, output, '.') //old fashion and char delimiter
sregex_token_iterator it(str.begin(), str.end(), regex, -1); // powerful
```
...
* linkedhashmap use list + map to implement
