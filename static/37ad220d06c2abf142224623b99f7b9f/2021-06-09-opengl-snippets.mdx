...
# Test Our Snippets
## Case 1
To draw a triangle to see if opengl snippet works. First `glheader` to include everthting and then type `glhellworld` to get the stub and fill the loop code with `glsimplevsfs`, `glshader`, `glvao`, `glvbo` and then `gldraw`. Totally time spent 1 min.
