...
```
## Android
Ok, it looks like there's some work to do for this couple when coming to android. Android requires nix-shell's `buildFHSUserEnv` which means it creates chroot sandbox to get path e.g. `/usr/bin/perl`. However, it conflicts with behavior of direnv. Therefore using both of them is not a good idea. In this case, it's like conda case: we need to use relative path solution. Or if setting up new env is hard, using docker is another option.

Actually, you download
