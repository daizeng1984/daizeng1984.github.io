...
+ use std::string_view replacing c style string or even c++ std::string, and get bonus like start_with(), substr() and better performance etc. view is **read only** but don't use in case when immutable is required like e.g. key for hashmap.
...
// GetOrDefault
(map.emplace(key, value)).first->first or second;
