...
# Why?
I know you should have some framework. But what if not or you need to do some low level API hack? You probably need to anyway find a toy example to get started.

Another point is: graphics API is designed for speed not easy to use (you can even say unreadable). To do a simple task, you need very strange function combination (e.g. genbuffer, bindbuffer ...) and you need to handle all the states that you probably cannot remember all.
