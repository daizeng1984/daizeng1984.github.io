...
* overflow handling is suggested as wrapping_* (mod) , checked_* (-> None if overflow) overflowing_* (->(val, boolean)) and saturating_* (clamp at min/max) (* can be add/substract etc.) You can see rust make newbie aware these issues instead of hiding like c/c++ did!!!


# [Nightly Official Guide](https://doc.rust-lang.org/nightly/book/index.html) (⭐⭐⭐⭐)
## types
```rust
