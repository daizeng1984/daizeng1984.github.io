...
* scope c/c++ similar in {}
* like c++ "hell,world" static, str -> c style string in stack ; String -> c++ std::string in heap and all primitives on stack follow the same pattern because they implement Copy trait. When you do assignment Rust will never automatically create “deep” copies unless you implemented copy. Rust choose to transfer ownership and invalidate the previous owner. If any type 
