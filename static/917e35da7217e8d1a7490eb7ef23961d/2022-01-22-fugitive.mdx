...
You can trace historical changes through `:0Gclog -- %` to get all commits containing current file. Of course `%` is just vim's syntax sugar for current file. You can put any file after the `--`. To diff any version in the quickfix window, I open a version (current buffer) and bind another [key]() in vim config to trigger a diff view of another version against current buffer.
