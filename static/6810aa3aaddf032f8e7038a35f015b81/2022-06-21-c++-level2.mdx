...
# Thread
You should use lock free or non-blocking concurrency. That means better stay away from mutex, semaphore etc. Why? cause you don't want to spend cycles on thread context management (put them sleep and signal them awake) and many bugs like dead lock. So you need to use atomic and spinlock. Careful design your datastructure. For competing resource e.g. multiple writers, spin lock it and gently pause; for assignment, CompareAndSwap; 
