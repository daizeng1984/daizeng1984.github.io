...
You can trace historical changes through `:0Gclog -- %` to get all commits containing current file. Of course `%` is just vim's syntax sugar for current file. You can put any file after the `--`. To diff the quickfix window, I bind another [key]() in vim config to diff with current buffer.
