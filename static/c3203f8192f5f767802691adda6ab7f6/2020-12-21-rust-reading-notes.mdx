...
* overflow handling is suggested as wrapping_* (mod) , checked_* (-> None if overflow) overflowing_* (->(val, boolean)) and saturating_* (clamp at min/max) (* can be add/substract etc.)
