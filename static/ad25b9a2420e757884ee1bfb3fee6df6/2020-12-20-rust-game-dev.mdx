...
Game developer tends to achieve maximum efficiency with limit hardware budget (a few milliseconds for 30fps, shooter game even higher). Bad habits foster to never care about error like segment fault and memory security. It makes sense if game hack is not that critical, gamer might just reboot and no one's bank account is going to be hacked.
