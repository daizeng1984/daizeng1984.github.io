...
Rust, as a more secure and yet efficient lang, recently catches my eyes and [some people](https://hub.packtpub.com/rust-as-a-game-programming-language-is-it-any-good/) believe it can replace (at least somewhat) the long-live C++ for game dev. Even though lang itself is not the one to blame for messy C++ game code, I still wish there'll be a cleaner/stricter language to branch from C++ (instead of merging tons of developer requests) so that developer doesn't need to create their own wheels, e.g. hack memory model etc.. C++ after 2013ish is full of sugars. It becomes more and more syntax-complicate that amounts to a new language. Too many historical burdens. Developers under C++ are not necessarily following strict rules they should which potentially leading to hard-to-find bugs like uninitialized resources, possibly lock threads etc..
