...
In my pov, Rust does the right balance between safety and efficiency. C++ can do that but requires trained and discipline programmers. The question will be down to whether extra 1% perf gain worth the trade-off. 

Currently, graphics lib e.g. [this](https://github.com/vulkano-rs/vulkano), is still under heavy dev, but it's just a matter of time.
...
## Godot
A bit cheating here. It's not rust written but there's wrapper for it.
