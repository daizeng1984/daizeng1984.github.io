...
## Git log
You can trace historical changes through `:0Gclog -- %` to get all commits containing current file. Of course `%` is just vim's sugar for current file. You can put any file after the `--`.
