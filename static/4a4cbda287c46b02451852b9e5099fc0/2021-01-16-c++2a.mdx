...
cout << arr.size(); //and basically vector's a few read method
//same value for every element?
arr.fill(5); // or std::to_array(), ancient c style can intial to 0 when {0}

// Trim, first string_view it
...
// Sort
sort(v.begin(), v.end(), [](const auto& x, const auto& y) -> bool {return x < y; });
