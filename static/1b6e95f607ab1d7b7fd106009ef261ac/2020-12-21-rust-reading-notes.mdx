...
* use std::rc::Rc is std::shared_ptr, Rc is immutable, however RefCell<T> can wrap it to make struct internal mutable. Note these 2 are not threadsafe, while Arc<Mutex<T>> is.
