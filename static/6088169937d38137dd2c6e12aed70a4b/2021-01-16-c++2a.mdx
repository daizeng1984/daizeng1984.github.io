...
## Practice Notes
Using it for some simple coding task and see what we need cover generally:
```cpp
using i32=int;
...
```
* std::array arr = {}; use it like const T[]
* trim
```cpp
int i = 0, j = v
while(
v.remove_prefix(std::min(v.find_first_not_of(" "), v.size()));
v.remove_suffix(std::max<unsigned int>(v.size() - v.find_last_not_of(" "), 0));
```
