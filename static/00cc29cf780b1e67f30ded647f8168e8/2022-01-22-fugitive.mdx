...
## Git mergetool
You can directly merge the conflicting file using `Gvdiffsplit`. It'll open as three way merge style if it has git merge tag (>>>>>>>>>). 3-way merge windows looks confusing but it's actually straightforward: From left to right is `Merge Target` or `Ours` ➡ `Working Copy` ➡ `Feature Branch` or `Theirs`.
