...
* & is stack ptr but with safer read/write + null guarantee and can be unsafely rawed to C++ ptr, Box<T\> is heap ref == new unique_ptr, Ref<T\> is heap share_ptr
* passing value == giveup ownership unless you implement copy/movable; passing ref only allows multiple immutable or 1 mutable
