...
* & is stack ptr but with safer read/write + null guarantee (reference_wrapper) and can be unsafely rawed to C++ ptr, Box<T\> is heap ref == new unique_ptr, Ref<T\> is heap share_ptr
* passing value == giveup ownership unless you has trait copy/movable (e.g. primitive type, &T, struct); passing ref means multiple immutable or 1 mutable
* copy is never deep (== c++ default assignment) unless you explicitly do clone
* str == C++ char[]; String == C++ std::string; [T;usize] == C++ T[]; Vec<T\> == C++ std::vector;
* &str is the common ground for String and str
* const == C++ macro or const
* data structure ...
...
| Box<T\> | Box<T\>::new(..) | in heap wrapped in obj == c++ unique_ptr | size overhead |
